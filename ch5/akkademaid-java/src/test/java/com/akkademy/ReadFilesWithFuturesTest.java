...
public class ReadFilesWithFuturesTest {
    ...
    public void shouldReadFilesWithFutures() throws Exception {
        ...
        List<CompletableFuture<Try<String>>> futures = (List) list
                .stream()
                .map(x -> CompletableFuture.supplyAsync(() -> ArticleParser.apply(TestHelper.file)))
                .collect(Collectors.toList());

        long start = System.currentTimeMillis();
        List<Try<String>> articles = com.jasongoodwin.monads.Futures.sequence(futures).get();
        System.out.println("articles count: " + articles.size());
        System.out.println("articles first: " + articles.get(0).orElse("error"));
